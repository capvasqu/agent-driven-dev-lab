// task-mcp — MCP server exposing the Kanban task operations as tools (Stage 3).
//
...
// (dogfooding). It speaks the MCP protocol over stdio.
//
// Note: an stdio MCP server must keep stdout clean for the protocol — never
// write logs to stdout here.
...
// Shared core: open the DB, ensure the schema, build the repository.
